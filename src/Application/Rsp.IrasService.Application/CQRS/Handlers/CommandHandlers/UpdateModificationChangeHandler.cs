...
/// </summary>
public class UpdateModificationChangeHandler(IProjectModificationService projectModificationService) : IRequestHandler<UpdateModificationChangeCommand>
{
    /// <summary>
    /// Processes the <see cref="UpdateModificationChangeCommand"/> and updates the specified modification status and changes.
    /// </summary>
    ...
    public async Task Handle(UpdateModificationChangeCommand request, CancellationToken cancellationToken)
    {
        // Delegate the removal to the application service. The service encapsulates the actual deletion logic.
        await projectModificationService.UpdateModificationChange(request.ModificationChangeRequest);
    }
}