...
/// <summary>
/// Repository abstraction for creating, querying and managing <see cref="ProjectModification"/>
/// aggregates and their related <see cref="ProjectModificationChange"/> entities.
/// </summary>
public interface IProjectModificationRepository
{
    ...
    /// </summary>
    /// <param name="projectModification">
    /// Fully populated modification entity to persist. The <see cref="ProjectModification.Id"/> may
    /// be pre-assigned (client) or generated (store) depending on implementation.
    /// </param>
    /// <returns>
    /// The created <see cref="ProjectModification"/> (including any store-assigned values).
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="projectModification"/> is null.
    /// </exception>
    ...
    /// </summary>
    /// <param name="projectModificationChange">
    /// The change entity to add. Must reference a valid parent <see cref="ProjectModification"/>
    /// via <see cref="ProjectModificationChange.ProjectModificationId"/>.
    /// </param>
    /// <returns>The created <see cref="ProjectModificationChange"/>.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="projectModificationChange"/> is null.
    /// </exception>
    ...
    /// Specification encapsulating the filtering criteria (typically by Id).
    /// </param>
    ...
    /// </summary>
    /// <param name="specification">
    /// Specification defining the filtering logic (e.g. by parent modification Id).
    /// </param>
    ...
    /// IrasId, ChiefInvestigatorName, ShortProjectTitle, SponsorOrganisation, FromDate
    /// (CreatedFrom), ToDate (CreatedTo), LeadNation (multi-value), ParticipatingNation
    /// (multi-value), ModificationTypes (multi-value), ReviewerId (when IncludeReviewerId = true).
    /// </param>
    ...
    /// Optional additional filter restricting results to a specific project record.
    /// </param>
    ...
    /// <summary>
    /// Retrieves a paged, sorted list of sponsor organisation user modifications matching the
    ...
    int GetModificationsBySponsorOrganisationUserCount(SponsorAuthorisationsModificationsSearchRequest searchQuery, Guid sponsorOrganisationUserId, string rtsId);
    ...
    /// </summary>
    /// <param name="Ids">
    /// Collection of modification identifiers (string form). Empty list returns empty result.
    /// </param>
    /// <returns>
    /// Matching <see cref="ProjectModificationResult"/> items (order not guaranteed unless enforced
    /// by implementation).
    /// </returns>
    ...
    /// <summary>
    /// Removes one or more <see cref="ProjectModificationChange"/> entities that satisfy the
    /// provided specification.
    /// </summary>
    /// <param name="specification">
    /// Specification that identifies the change(s) to remove. Must not be null.
    /// </param>
    ...
    /// </exception>
    /// <remarks>
    /// Implementations should treat this as an idempotent operation: if no entities match, the
    /// method completes without error. Implementations may choose to remove all matches or enforce
    /// that the specification uniquely identifies a single entity.
    /// </remarks>
    ...
    /// specification. Only updates fields that are explicitly set (non-null) in the specification.
    /// If no matching entity is found, the method completes without making any changes.
    /// </summary>
    /// <param name="specification">The specification used to locate the modification to update.</param>
    /// <param name="projectModification">
    /// The <see cref="ProjectModification"/> entity with updated values.
    /// </param>
    Task UpdateModification(ISpecification<ProjectModification> specification, ProjectModification projectModification);

    /// <summary>
    /// Updates an existing <see cref="ProjectModificationChange"/> entity. Only updates fields that
    /// are explicitly set (non-null) in the change object. If the change is null, or if no change
    /// matches the identifier, the method completes without making any changes.
    /// </summary>
    /// <param name="modificationChange">
    /// The <see cref="ProjectModificationChange"/> entity with updated values. Identifier must be
    /// set to match the change to update.
    /// </param>
