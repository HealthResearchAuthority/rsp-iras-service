...
/// <summary>
///     Covers the tests for SaveModificationDocuments method
/// </summary>
public class SaveModificationDocuments : TestServiceBase<RespondentService>
{
    private readonly RespondentRepository _personnelRepository;
    private readonly RspContext _context;

    public SaveModificationDocuments()
    {
    ...
    /// </summary>
    [Theory, AutoData]
    public async Task Persists_ModificationDocuments(List<ModificationDocumentDto> request)
    {
        // Arrange
        ...
        // Act
        await Sut.SaveModificationDocumentResponses(request);

        // Assert
        (await _context.ModificationDocuments.CountAsync()).ShouldBe(request.Count);
    }
}