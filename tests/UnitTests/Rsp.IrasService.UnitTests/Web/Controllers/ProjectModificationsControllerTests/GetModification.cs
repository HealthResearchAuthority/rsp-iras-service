...
public class GetModificationControllerTests : TestServiceBase<ProjectModificationsController>
{
    ...
    {
        // Act
        var result = await Sut.GetModification("");
    ...
    public async Task GetModification_Sends_Query_And_Returns_Response()
    {
        ...
        var expected = new ModificationResponse { Id = Guid.NewGuid(), ProjectRecordId = "PR-1", ModificationIdentifier = id, Status = "Draft", CreatedBy = "u", UpdatedBy = "u", CreatedDate = DateTime.UtcNow, UpdatedDate = DateTime.UtcNow };
        mediator.Setup(x => x.Send(It.IsAny<GetModificationQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(expected);

        // Act
        var res = await Sut.GetModification(id);
